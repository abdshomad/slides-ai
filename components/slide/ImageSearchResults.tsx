...
const ImageSearchResultCard: React.FC<{ result: ImageSearchResult; onSelect: () => void }> = ({ result, onSelect }) => {
    return (
        <button
            onClick={onSelect}
            className="w-full h-full bg-slate-300 dark:bg-slate-700 rounded-md overflow-hidden group relative transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-pink-500/30"
...
const ImageSearchResults: React.FC<ImageSearchResultsProps> = ({ results, onSelect }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center p-2 text-center bg-slate-200/50 dark:bg-slate-800/50">
            <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-2">Choose an Image</h4>
